...
class AbsolutePositionalEmbedding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        print(f"Debug: x.shape before unpacking!!! = {x.shape}")  # Added for debugging
        batch_size, seq_len = x.size()  # Ensure x has 2 dimensions
...
#     def forward(self, x: torch.Tensor) -> torch.Tensor:
#         print(f"Debug: x.shape before unpacking = {x.shape}")
