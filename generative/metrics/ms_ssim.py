...
class MultiScaleSSIMMetric(RegressionMetric):
    ...
    def __init__(
        ...
        data_range: float = 1.0,
        kernel_type: KernelType | str = KernelType.GAUSSIAN,
        kernel_size: int | Sequence[int, ...] = 5,  # Reduced from 11 to 5
        kernel_sigma: float | Sequence[float, ...] = 1.0,  # Adjusted to match smaller kernel
        k1: float = 0.01,
        k2: float = 0.03,
        weights: Sequence[float, ...] = (0.2856, 0.3001, 0.4143),  # Reduced to 3 scales
        reduction: MetricReduction | str = MetricReduction.MEAN,
        get_not_nans: bool = False,
    ) -> None:
